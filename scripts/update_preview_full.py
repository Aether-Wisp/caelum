...
<head>
    <meta charset="UTF-8">
    <title>Caelum - React 实时预览 (Linear 风格)</title>
    <!-- Tailwind CSS -->
    ...
    <style>
      /* CSS variables defined in DESIGN.md */
      :root {
      ...
      .linear-input:focus {
        border-color: var(--color-text-pri);
        box-shadow: 0 0 0 1px var(--color-text-pri);
      }
      .linear-card {
...
<body class="bg-[#f8f9fa] m-0 p-0 overflow-hidden text-[#111827]">
    ...
    <script type="text/babel">
        ...
        function Badge({ type, children }) {
           let colors = { text: 'var(--color-text-pri)', bg: 'var(--color-secondary)' };
           if (type === 'severe' || type === 'error') colors = { text: 'var(--color-error)', bg: '#fef2f2' };
           if (type === 'high' || type === 'warning') colors = { text: 'var(--color-warning)', bg: '#fffbeb' };
           if (type === 'low' || type === 'success') colors = { text: 'var(--color-success)', bg: '#ecfdf5' };
           
           return (
             <span style={{ fontSize: '12px', padding: '2px 8px', backgroundColor: colors.bg, color: colors.text }} className="rounded-md font-medium border border-current opacity-80">
                {children}
             </span>
           );
        }
...
"));
        ...
        function App() {
          ...
          let contentTitle = "安全测试任务";
          if (activeTab === 'payload') contentTitle = "Payload 生成";
          if (activeTab === 'history') contentTitle = "历史日志";
          ...
          return (
             <div style={{ backgroundColor: '#f8f9fa' }} className="flex h-screen w-screen font-sans antialiased text-[#111827]">
              ...
              <aside style={{ width: '260px', borderRight: '1px solid var(--color-border)' }} className="flex flex-col flex-shrink-0 bg-[#ffffff] z-10">
                <div className="h-[64px] flex items-center px-6 border-b border-[var(--color-border)] mb-4">
                  <div style={{ width: '28px', height: '28px', backgroundColor: 'var(--color-primary)', borderRadius: '6px' }} className="flex items-center justify-center text-white mr-3">
                    <span style={{ fontSize: 'var(--text-base)', fontWeight: '600' }}>C</span>
                  </div>
                  <span style={{ fontSize: '16px', fontWeight: '600', letterSpacing: '-0.02em' }}>Caelum</span>
                </div>
              ...
              <div className="flex-1 flex flex-col min-w-0 bg-[#f8f9fa] overflow-hidden">
                {/* Header Navbar */}
                <header style={{ height: '64px', borderBottom: '1px solid var(--color-border)' }} className="bg-[#ffffff] flex items-center justify-between px-8 flex-shrink-0 z-10 shadow-sm shadow-[#00000002]">
                  <h1 style={{ fontSize: 'var(--text-base)' }} className="font-semibold text-[var(--color-text-pri)] tracking-tight">{contentTitle}</h1>
                  
                  <div className="flex items-center space-x-5">
                    <button className="text-[var(--color-text-sec)] hover:text-[var(--color-text-pri)] transition-colors relative">
                        <Icon name="bell" size={20} />
                        <span className="absolute -top-1 -right-0.5 w-[6px] h-[6px] bg-[var(--color-error)] rounded-full ring-2 ring-white"></span>
                    </button>
                    <div style={{ padding: '3px 12px 3px 3px', border: '1px solid var(--color-border)', borderRadius: '24px' }} className="flex items-center cursor-pointer hover:bg-gray-50 transition-colors">
                        <div style={{ width: '28px', height: '28px', backgroundColor: 'var(--color-secondary)', borderRadius: '50%' }} className="flex items-center justify-center text-[var(--color-text-pri)] text-xs font-semibold mr-2 border border-[var(--color-border)]">
                            AD
                        </div>
                        <span style={{ fontSize: 'var(--text-sm)' }} className="font-medium text-[var(--color-text-pri)]">Administrator</span>
                    </div>
                  </div>
                </header>
                ...
                <main className="flex-1 overflow-y-auto p-6 md:p-8">
                  <div className="max-w-[1400px] mx-auto h-full">
                    ...
                    {activeTab === 'payload' && <PayloadContent />}
...
print("react-preview.html updated with full functional views.")
